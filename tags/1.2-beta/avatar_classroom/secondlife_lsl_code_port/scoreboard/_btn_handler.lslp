...
clear(){
        ...
        llMessageLinked(LINK_SET, XY_QUIZ_CHANNEL, "                    ", "0");
        llMessageLinked(LINK_SET, XY_GAMEID_CHANNEL, "                    ", "0");
...
integer getIndex(string view){
            ...
            integer found = llListFindList(viewData,[llStringTrim(view,STRING_TRIM)]);      
     ...
     state_entry() {
        ...
        for (counter=0;counter<300;counter++){
         ...
         //define commands that are executed when an NEXT PAGE or PREV PAGE button is pressed
          ...
          viewData+= ["Quizzes",0,0,"quiz->getQuizzes&index={index}&maxitems=9",PLUGIN_CHANNEL];          
          ...
          viewData+= ["Team Top Scores",0,0, "awards->getTeamPlayerScores&index={index}&maxitems=9&sortmode=name&currency=Credits",PLUGIN_CHANNEL];
      ...
      clearHighlights();
        ...
        llMessageLinked(LINK_SET, XY_TEXT_CHANNEL, blanks, "0");
             ...
             if (button=="Config Tab"){
                    ...
                    //Display Buttons                                                                                      
                    if (currentAwardId!=-1)
                        llMessageLinked(LINK_SET, UI_CHANNEL,"CMD:BTN DISPLAY|BUTTON:s1|TEXTURE:btn_scavengerhunt|DISPLAY:HIDE|POWER:OFF",NULL_KEY); else
                    ...
                    llMessageLinked(LINK_SET, UI_CHANNEL,"CMD:BTN DISPLAY|BUTTON:s2|TEXTURE:btn_http|DISPLAY:SHOW|POWER:ON",NULL_KEY);
                    llMessageLinked(LINK_SET, UI_CHANNEL,"CMD:BTN DISPLAY|BUTTON:s3|TEXTURE:blank|DISPLAY:HIDE|POWER:OFF",NULL_KEY);
                    llMessageLinked(LINK_SET, UI_CHANNEL,"CMD:BTN DISPLAY|BUTTON:s4|TEXTURE:blank|DISPLAY:HIDE|POWER:OFF",NULL_KEY);
                    ...
                    }else llOwnerSay("Not a facilitator");
              }//endif Config Tab
              else
              /**************************************************
              *    Buttons - Students
            ...
            **************************************************/
            if (currentTab=="Groups Tab"){
                ...
                if (button=="s0"){
                  ...
                  //Print and center a title                         
                  left("Team Top Scores");
                  //execute sloodle api function (sloodle_plugin_users.lsl will react to the sloodle_api http response)                                               
                  llMessageLinked(LINK_SET, PLUGIN_CHANNEL, "awards->getTeamPlayerScores&currency=Credits"+authenticatedUser+"&sloodlemoduleid="+(string)currentAwardId+"&index="+(string)index+"&maxitems=9&sortmode=name", NULL_KEY);
                  //instructional text - 
                  llMessageLinked(LINK_SET, XY_DETAILS_CHANNEL, currentAwardName, NULL_KEY);                                               
                }//endif button=="s0"
               else
                  ...
                  llMessageLinked(LINK_SET, PLUGIN_CHANNEL, "awards->getAwardGrps"+authenticatedUser+"&sloodlemoduleid="+(string)currentAwardId+"&index="+(string)index+"&maxitems=9", NULL_KEY);
                               }
              }//endif button=="s1"    
              else
              if (button=="s2"){
          ...
          if (button=="s2"){
                   if (isFacilitator(avname)){   
                    llMessageLinked(LINK_SET, XY_DETAILS_CHANNEL,"Resetting HttpIn URL", NULL_KEY);
                    llMessageLinked(LINK_SET, UI_CHANNEL,"CMD:GET URL", NULL_KEY);
                   //clear highlight}
                   }else llOwnerSay("Not a facilitator");
              }
            /*
               if (button=="s1"){//scavenger hunt
                ...
                llMessageLinked(LINK_SET, XY_TEXT_CHANNEL, "A sloodle_config has been added to the Awards Cup.  Please add the following line to it:  set:sloodlemoduleid|"+(string)currentAwardId, NULL_KEY);         
