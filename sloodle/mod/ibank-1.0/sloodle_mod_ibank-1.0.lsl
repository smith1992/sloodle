...
debugMessage(string message){
  //llOwnerSay(" ~~~StipendGiver_mod script debug message ~~~ " + message);
}
...
integer sloodle_handle_command(string str) 
{
    ...
    else if (name == "set:defaultstipend") defaultStipend  = (integer)value1;
    else if (name == "set:totalStipends") totalStipends  = (integer)value1;
    else if (name == "set:numStudents") numStudents  = (integer)value1;
    else if (name == "set:iCurrencyType") iCurrencyType  = (string)value1;
    else if (name == "SCOREBOARD_CHANNEL") SCOREBOARD_CHANNEL  = (integer)value1;
...
default{
     state_entry()
        {
          myKey=llGetKey();
          //llOwnerSay("My key is: "+(string)myKey); 
  ...
  run_time_permissions (integer perm)
    ...
    on_rez(integer start_param){
        //make nice startup sound
         llMessageLinked(LINK_SET, SOUND_CHANNEL,"COMMAND:PLAYSOUND|SOUND_UUID:676bd8f1-a061-72f4-b56c-93408f9cba46|SCRIPT_NAME:soundPlayer 1|VOLUME:0.8", NULL_KEY);         
        llResetScript();   
    }    
    ...
    state_entry()
{
        ...
        llMessageLinked(LINK_SET, SOUND_CHANNEL,"COMMAND:PLAYSOUND|SOUND_UUID:676bd8f1-a061-72f4-b56c-93408f9cba46|SCRIPT_NAME:soundPlayer 2|VOLUME:0.8", NULL_KEY);
        ...
        llMessageLinked(LINK_SET, SOUND_CHANNEL,"COMMAND:PLAYSOUND|SOUND_UUID:50091bcd-d86d-3749-c8a2-055842b33484|SCRIPT_NAME:soundPlayer 3|VOLUME:0.8", NULL_KEY);
    }
       ...
       //    10+)    studentRowData                     
        ...
        if (status == -321) {
            ...
            llDialog(llGetOwnerKey(llGetKey()),"Please rez a RegEnrol booth first, and enrol your avatar to your moodle site",["Reset"],ADMIN_CHANNEL);        
               ...
               // llSay(0,"Line 1) origionalSenderUuid: " + (string)origionalSenderUuid);
                //line 2      
                string command = llList2String(lines,2);
                //llSay(0,"Line 2) Command: " + (string)command);       
                //line 3
                numStudents = llList2Integer(lines,3);
               // llSay(0,"Line 3) numStudents: " + (string)numStudents);
                fullCourseName = llList2String(lines,4);
                //llSay(0,"Line 4) coursefullname: " + (string)fullCourseName);
                //line 4
                sloodleName = llList2String(lines,5);
                //llSay(0,"Line 5) sloodleName :" + (string)sloodleName);  
                //line 5
                sloodleIntro = llList2String(lines,6);
                //llSay(0,"Line 6) sloodleIntro :" + (string)sloodleIntro); 
                //line 6
                defaultStipend = llList2Integer(lines,7);
                
                //llSay(0,"Line 7) defaultStipend :" + (string)defaultStipend);
                //line 7
                ...
                iCurrencyType = llList2String(lines,9);
                 //llSay(0,"Line 9) iCurrencyType :" + (string)iCurrencyType);  
...
state readOneLine{
    ...
    state_entry() {    
        ...
        //send dataline to the current memory channel
            ...
            playerData = llParseString2List(dataLine,["|"],[]);
            ...
            llMessageLinked(LINK_THIS,IBANK_DATA_CHANNEL, dataLine,llGetKey());
        }
...
{ 
    state_entry()
    {
       llMessageLinked(LINK_SET, SOUND_CHANNEL,"COMMAND:PLAYSOUND|SOUND_UUID:da0e311f-42a2-3b44-d96e-aeb19789999e|SCRIPT_NAME:soundPlayer|VOLUME:0.8", NULL_KEY);// congrats chime sound
    ...
    link_message(integer sender_num, integer num, string str, key id) {
       debugMessage("*****stipendgiver main script  got linked message : "+str);
        ...
        }else if ((command=="GIVE MONEY")&& (id==llGetKey())){
