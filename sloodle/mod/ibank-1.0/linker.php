<?php        
    ...
    * @param int CHUNK_SIZE The number of users we will upload into SL at a time
    * @param sCourseObj  Used to access course specific information
    * @param
    */
    ...
    $ibankObj = new Ibank((int)$sCourseObj->cm->instance);
    ...
    */
           ...
           //  | user debits
            ...
            $avList= array();
             foreach ($avatarList as $u){
                   ...
                   $av->avname = $u->avname;
                   $av->uuid = $u->uuid;
                   $av_balanceDetails= $ibankObj->ibank_getBalanceDetails($u->userid);
    ...
    /*
    * addDefaultStats - Every time the linker.php script sends data back to the iBank it includes basic
    * stats of the course and this module, so the ibank can stay updated in SL
    ...
    * line 8) api  do we pass through to ibank, or an ibank module?  YES - SET TO TRUE
    * 
    ...
    */
    ...
    function addDefaultStats(){
        ...
        global $sCourseObj;
        global $ibankObj;  
            $ibankId = $sloodle->request->optional_param('sloodlemoduleid'); 
            $userList = $sCourseObj->userList;
            ...
            $sloodle->response->add_data_line("IBANK NAME:".$sloodleName); //line 5   
            //add sloodle intro
            $sloodleIntro= $sloodle->module->get_intro();
            $sloodle->response->add_data_line("IBANK INTRO:".$sloodleIntro); //line 6
            //add iCurrencyType
            $iCurrencyType = $ibankObj->sloodle_ibank_instance->icurrency;
            ...
            //otherwise if it is false, the ibank uses the response
            $sloodle->response->add_data_line("IBANKID:".$ibankId);               //line8
    }
    /*
    * getFieldData - string data sent to the ibank has descripters built into the message so messages have a context
    ...
    */
    ...
    //*****************************************************************************************************
    //Execution point:
    //retreive the command sent from the ibank
    $command = $sloodle->request->optional_param('command');
    ...
    switch ($command){
        ...
        *
        ...
        3* NUM USERS:22
        4* COURSE NAME:iBank Demo
        5* IBANK NAME:ibank: get your ipoints!
        6* IBANK INTRO:Test
        7* ICURRENCY:iPoints
        8* IBANKID:114
        */    
        case "REGISTER":
            $data=$sloodle->request->optional_param('xmlchannel'); 
            $ibankObj->setXmlChannel($data);
        ...
        if (!$avuser->load_avatar($avuuid, null)) {
           ...
           $dataLine.="|0|0|0";
            ...
            if (!$avuser->is_enrolled($sloodle->course->get_course_id())){
                ...
                $sloodle->response->set_status_descriptor('USER NOT ENROLLED IN COURSE');//line 0 
                $dataLine.="|0|0|0";
            }else {
                //user is enrolled                                           
                //get credits and debits
                
                $rec= $ibankObj->ibank_getBalanceDetails($avuser->get_user_id());
                if (!empty($rec)) {   
                    $balance=$rec->balance;
                    $credits = $rec->credits;                     
                    $debits = $rec->debits;
        ...
        * 
        * The message we will send back to the ibank is:
        ...
        */
        ...
        if (!$avuser->load_avatar($avuuid, null)) {
           ...
           $dataLine.="|NULL";
            ...
            if (!$avuser->is_enrolled($sloodle->course->get_course_id())){
                ...
                $rec= $ibankObj->getLastTransaction($avuuid,"attendance"); 
                if (!empty($rec)) {                        
                    $sloodle->response->set_status_code(1);             //line 0    
                    $sloodle->response->set_status_descriptor('OK');    //line 0 
                    $dataLine.="|".$rec->idata; 
                    ...
                    $sloodle->response->set_status_descriptor('TRANSACTION NOT FOUND');    //line 0 
        ...
        $rec = $ibankObj->ibank_getBalanceDetails($moodleId);
        $balance = $rec->balance;
        
     //send message back to ibank
       //   LINE   MESAGE
       //    0)     1 | OK
       ...
       //    6)     sloodle intro 
       //    7)     iCurrencyType 
       //    8)     moduleid - module id of the ibank
