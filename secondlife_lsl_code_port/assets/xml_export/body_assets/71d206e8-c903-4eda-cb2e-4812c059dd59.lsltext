...
integer debugCheck(){
 ...
 debug(string str){
            if (llList2Integer(llGetPrimitiveParams([PRIM_MATERIAL]),0)==PRIM_MATERIAL_FLESH){
                llOwnerSay(llGetScriptName()+" "+(string)llGetFreeMemory()+" " +str);
           }
        }
// *************************************************** SLOODLE TRANSLATION
...
sendCommand(string str,string httpid){
        ...
        cmdStr = llGetSubString(str, 0, varStartIndex-1);
        cmdLine = llParseString2List(cmdStr,["->"],[]); //plugin:groups,function:checkEnrols
    ...
    if (channel==SLOODLE_CHANNEL_OBJECT_DIALOG){
         ...
         http_response(key id,integer status,list meta,string body) {
                       ...
                       string responseLine = llList2String(lines, 1);
                                  debug("********** "+llGetScriptName()+" SERVER RESPONSE "+s(responseLine)+" *********************");                  
                                  result= llParseString2List(body, ["\n"], []); //parse the message into a list
                                  len = llGetListLength(result);
                                j=0;
                                for (j=0;j< len;j++){
                                       debug("* "+(string)j+") "+llList2String(result,j ));
                                }//end for
                                debug("******************************************************\n");
                                result=[];
        ...
        //retrieve lines from the http body   
         if ((id == http)){
              ...
              debug("sending response "+body+" ----- of http on: "+(string)PLUGIN_RESPONSE_CHANNEL);
              http=NULL_KEY;
            }
         else
         if ((id == http2)){ llMessageLinked(LINK_SET, PLUGIN_RESPONSE_CHANNEL+5, body, NULL_KEY);
             debug("sending response "+body+" ----- http2 on: "+(string)(PLUGIN_RESPONSE_CHANNEL+5));
                       http2=NULL_KEY;
                }
         else {
             debug("sending response "+body+" -----  http3 on: "+(string)(PLUGIN_RESPONSE_CHANNEL+6));
