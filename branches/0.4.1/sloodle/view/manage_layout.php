...
///////////////////
    ...
    // Log the view
    add_to_log($courseid, 'course', 'manage layout', "mod/sloodle/view/manage_layout.php?courseid={$courseid}", "$course->id");
   
    // Is the user allowed to edit this course?
    require_capability('moodle/course:update', $course_context);

    // Display the page header
    $navigation = "<a href=\"{$CFG->wwwroot}/mod/sloodle/view/view_course.php?id={$course->id}\">".get_string('managelayout','sloodle')."</a>";
    print_header_simple(get_string('managelayout','sloodle'), "", $navigation, "", "", true, '', navmenu($course));
...
//------------------------------------------------------    
    ...
    if (isset($_POST['courseid'])) {
       ...
       /// Update
          ...
          if ($result) {
             $next = 'manage_layout.php?courseid='.$courseid;
...
//------------------------------------------------------    
    ...
    if (count($layout_names) > 0) {
        ...
        foreach($layout_names as $lid=>$ln) {
            ...
            echo "<tr><td>$ln</td><td>$en_cnt</td><td><a href=\"manage_layout.php?courseid=".$courseid."&layoutid=".$lid."\">Edit</a></td></tr>";
        }
        echo "<tr><td colspan=\"2\"> </td><td><a href=\"manage_layout.php?courseid=".$courseid."&layoutid=0\">Add</a></td></tr>";
        echo "</table>";
        echo "<center>";
...
//
    ...
    // Create a form
    echo "<form action=\"manage_layout.php\" method=\"POST\">\n";
