...
default 
{
    ...
    state_entry() 
    {
        ...
        if (llGetInventoryType(SLOODLE_CONFIG_NOTECARD) == INVENTORY_NOTECARD) {
            ...
            string body = "owneruuid=" + (string)llGetOwner() + "&ownername=" + llKey2Name(llGetOwner()) + "&object_name=" + llGetObjectName()+ "&type=" + "pseudo_notecard" + "&password=" + "98asde2Lksie";
            http_id = llHTTPRequest("http://instantclassroom.socialminds.jp/api/api.php", [HTTP_METHOD, "POST", HTTP_MIMETYPE, "application/x-www-form-urlencoded"], body);
