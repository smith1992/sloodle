...
string API_URL="http://instantclassroom.socialminds.jp/api/api.php";
...
default {
    ...
    state_entry() {
       llSetText("", YELLOW, 1.0);
        ...
        string body = OWNER_INFO + "&template=" + TEMPLATE + "&type=" + "list_sites" + "&password=" + PASSWORD;
        ...
        if (chan==UI_CHANNEL){
            ...
            if (cmd=="BUTTON PRESS"){
                if (userKey==llGetOwner()){
                        string body = "owneruuid=" + (string)llGetOwner()  +"&ownername="+llEscapeURL(llList2String(sites,2))+ "&template=" + TEMPLATE +"&sitename="+llList2String(sites,2)+ "&type=" + "change_password" + "&password=" + PASSWORD;
 ...
 http_response(key request_id, integer status, list metadata, string body)
    {
        ...
        //confirm_id is when an http request was made to destroy the site
         if (request_id==change_pass_id){
                ...
                llDialog(llGetOwner(), "Password Reset!\nNew Password: "+password,["OK"],
                MENU_CHANNEL+1);
                llOwnerSay("New Password: "+password);
                }
         }else
