// Address to email blog entries to.
// If you leave this empty, you will be prompted to chat it when you use the Toolbar.
string DESTINATION_EMAIL = ""; // e.g.: "abc.def@blogger.com"


// Sloodle Email Blog HUD
// Allows SL users in-world to write blog entries which get emailed.
// Part of the Sloodle project (www.sloodle.org)
//
// Destination email address can be specified in script, or will be prompted for at run-time otherwise.
//
...
float HTTP_TIMEOUT = 20.0; // Time to wait for an HTTP response (increase since we're dealing with emails)
...
integer MAX_BLOG_BODY_LENGTH = 1001; // This may change if our target is Twitter
integer DEFAULT_MAX_LENGTH = 800;
integer USING_TWITTER = FALSE; // We'll detect this from the given email address
...
{
    return; // Visibility cannot be changed for emailed blog posts
    dlglisten_channel = 0;
        ...
        if (num == SLOODLE_CHANNEL_OBJECT_DIALOG) {
            ...
            if (eof == TRUE) {
                if (isconfigured == TRUE) {
                    sloodle_translation_request(SLOODLE_TRANSLATE_OWNER_SAY, [0], "configurationreceived", [], NULL_KEY, "");
                    state getemail;
                } else {
                    // Go all configuration but, it's not complete... request reconfiguration
                    sloodle_translation_request(SLOODLE_TRANSLATE_OWNER_SAY, [0], "configdatamissing", [], NULL_KEY, "");
...
/// GET EMAIL ///
// Prompt for the email address is necessary
state getemail
{
    state_entry()
    {
        // Do we already have an email address?
        ...
        // Prompt for it
        sloodle_translation_request(SLOODLE_TRANSLATE_OWNER_SAY, [], "enteremail", ["7"], null_key, "toolbar");
        llListen(7, "", llGetOwner(), "");
    }
    
    state_exit()
    {
        // Report on which email address we're using
        sloodle_translation_request(SLOODLE_TRANSLATE_OWNER_SAY, [], "usingemail", [DESTINATION_EMAIL], null_key, "toolbar");
        // Set our maximum size as appropriate
        if (llSubStringIndex(DESTINATION_EMAIL, "@twittermail.com") >= 0) {
            USING_TWITTER = TRUE;
            MAX_BLOG_BODY_LENGTH = 160;
            sloodle_translation_request(SLOODLE_TRANSLATE_OWNER_SAY, [], "targettwitter", [(string)MAX_BLOG_BODY_LENGTH], null_key, "toolbar");
        } else {
            USING_TWITTER = FALSE;
            MAX_BLOG_BODY_LENGTH = DEFAULT_MAX_LENGTH;
        }
    }
    
    listen(integer channel, string name, key id, string msg)
    {
        // Make sure it's from the owner
        if (id != llGetOwner()) return;
        
        // Check that the message looks like an email address
        msg = llStringTrim(msg, STRING_TRIM);
        if (llSubStringIndex(msg, "@") < 0) {
            sloodle_translation_request(SLOODLE_TRANSLATE_OWNER_SAY, [], "entervalidemail", [], null_key, "toolbar");
            return;
        }
        
        // Store it and move on
        DESTINATION_EMAIL = msg;
        state ready;
    }
...
{   
    state_entry()
    {
        // If we're using Twitter, skip the subject line
        if (USING_TWITTER) {
            sloodle_translation_request(SLOODLE_TRANSLATE_OWNER_SAY, [], "twitterskipsubject", [], null_key, "toolbar");
            blogsubject = "Twitter";
...
{   
    state_entry()
    {
        ...
        // Set the initial blog body length to the length of the subject line, unless we're using Twitter
        if (USING_TWITTER) blogbodylength =  0;
        else blogbodylength = llStringLength(blogsubject);
...
{   
    state_entry()
    {
        ...
        body += "&sloodledestinationemail=" + DESTINATION_EMAIL;
    ...
    http_response(key request_id, integer status, list metadata, string body)
    {
        ...
        if (statuscode <= -300 && statuscode > -400) {
            ...
            // Failed to send an email to the blog
            llOwnerSay("ERROR: failed to send email to your destination blog");
            sloodle_debug(body);
            state default;
            return;
