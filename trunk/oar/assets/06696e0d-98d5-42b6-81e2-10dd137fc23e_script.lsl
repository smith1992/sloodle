    ...
    listen( integer channel, string name, key id, string message)
    {
        ...
        if (channel == SLOODLE_CHANNEL_AVATAR_DIALOG) {
            ...
            if (message == "0") {
                ...
                string chatroomurl = sloodleserverroot + "/mod/chat/view.php?id="+(string)sloodlemoduleid;
                sloodle_translation_request(SLOODLE_TRANSLATE_SAY, [0], "webintercom:anouncechatroom", [chatroomurl], NULL_KEY, "webintercom");
                llLoadURL(id, "Goto Moodle Chat Room", chatroomurl);
                ...
                // Modified by Fumi.Iseki
                string chatroomurl = sloodleserverroot + "/mod/chat/view.php?id="+(string)sloodlemoduleid;
                sloodle_translation_request(SLOODLE_TRANSLATE_IM, [id], "webintercom:enterchatroom", [chatroomurl], id, "webintercom");
                llLoadURL(id, "Goto Moodle Chat Room", chatroomurl);
