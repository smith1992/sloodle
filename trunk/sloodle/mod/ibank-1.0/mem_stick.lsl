...
*  
*  PURPOSE
*  The Purpose of this script is store up to 12 students downloaded rows of student data
...
*
*  Memsticks are responsible for handling user clicks on the iBank. Since this memstick is only responsible
*  for a max of 12 users, it will only display a two page menu of users, and functions for those users.  If 
*  a user clicks ">>" or "<<" in the menu, this mem_stick may go into waiting state, and send out a message 
*  for another mem_stick to control. During the loading stage, when this mem_stick becomes full, another mem_stick will
*  start loading the remaining users into memory.  Thus, mem_sticks act as a dynamic memory solution  
**********************************************************************************************/
...
integer userIndex;
integer result;
integer SCOREBOARD_CHANNEL=880;
...
{
    bits = llParseString2List(str,["|"],[]);
    numbits = llGetListLength(bits);
...
string getStats(integer userIndex){
        userName = llList2String(avatarNameList,userIndex);
        channel = llList2Integer(channelList,userIndex);
        //set menu text
        if (iCurrencyType=="Lindens"){
        ...
        newValue = llList2Integer(modifiedStipendAmounts,userIndex) + llList2Integer(avatarAllocationList,userIndex);
        savedValue =llList2Integer(avatarAllocationList,userIndex);
...
integer sendCommand(string command, string data,key senderUuid){
     ...
     //llSay(0,"mem stick: Freemem: " + (string)llGetFreeMemory());
       // llSay(0,"HttpScript:  sending this to linker.php\n"+sloodleserverroot + SLOODLE_STIPENDGIVER_LINKER+"?"+body);
...
list getMenu(integer cIndex){
    ...
    while ((tempIntA< cIndex+MAX_BUTTONS) && (tempIntA < memStickNumStudents)){
        ...
        //llSay(0,"adding: " +llGetSubString(llList2String(avatarNameList,tempIntA), 0, 11));
   ...
   //show previous button only if we are not on the first page, or if we are in a memory stick other than the first memory stick
    ...
    if (((masterMenuIndex + MAX_BUTTONS) < numStudents))       
            tempList1 += ">>";
    tempList1 += "(-~ MENU ~-)";
    //llSay(0,(string)(tempList + menuButs1 + tempList1));
...
default{  
    ...
    //    llSay(0,(string)myNum+ " waiting for initialization"+(string)llGetFreeMemory());
    ...
    link_message(integer sender_num, integer script_channel, string str, key id) {       
       ...
       linkCommand = llList2String(linkMessageList,0);
        if (script_channel == MY_SCRIPT_CHANNEL){     
                    ...
                    memStickNumStudents++;
                  //llSay(0,(string)memStickNumStudents +"-----------------> mem stick #"+(string)myNum+" READ: "+llList2String(linkMessageList,2) + " FreeMem is: " + (string)llGetFreeMemory());
                    ...
                    if (command=="Set Default"){
                        setDefaultStipendDialog(avuuid);
                           ...
                           llSay(SCOREBOARD_CHANNEL,"Reset");
                            ...
                            llMessageLinked(LINK_SET, SOUND_CHANNEL,"COMMAND:PLAYSOUND|SOUND_UUID:348ed543-308e-02fd-6769-ab4a1b56a9ad|SCRIPT_NAME:soundPlayer 4|VOLUME:0.8", NULL_KEY);// ECHOBEL2 chime sound
...
//                    debugMessage("masterMenuIndex: "+(string)masterMenuIndex+ " currentMenuIndex: "+(string)currentMenuIndex +" prevMenuButtons: "+(string)prevMenuButtons);
                    ...
                    displayUserMenu(command,avuuid);
                }
            }else
            if (channel >= USER_CHANNELS){//messge came from a response from admin from userMenu
                ...
                if (command=="Modify")
                    ...
                    amount = (integer)command;

                    //this will change the usersModification to the amount sent in the menu
                   

                    userIndex = llListFindList(channelList,[channel]);
                    avatarName = llList2String(avatarNameList,userIndex);
                    modAmount = llList2Integer(modifiedStipendAmounts,userIndex)+llList2Integer(avatarAllocationList,userIndex);
