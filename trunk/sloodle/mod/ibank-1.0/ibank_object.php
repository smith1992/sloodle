<?php
  ...
  class Ibank{
      ...
      */
      function Ibank($sloodleId){          
          $this->sloodle_ibank_instance = get_record('sloodle_ibank','sloodleid',$sloodleId); 
          $this->sloodleId=$sloodleId;                        
          $this->transactionRecords = $this->ibank_getTransactionRecords();          
      }
      /**
     * @method ibank_makeTransaction
     ...
     */
      ...
      function setXmlChannel($xmlChannel){        
          $this->sloodle_ibank_instance->xmlchannel=$xmlChannel; 
          $this->timeupdated = time();
          return update_record('sloodle_ibank', $this->sloodle_ibank_instance);
        
      }
      function setAmount($amount){
          $this->sloodle_ibank_instance->amount=$amount; 
          $this->timeupdated = time();
          return update_record('sloodle_ibank', $this->sloodle_ibank_instance);
        
      }
      function setIcurrency($icurrency){
        $this->sloodle_ibank_instance->icurrency=$icurrency; 
        $this->timeupdated = time();
        return update_record('sloodle_ibank', $this->sloodle_ibank_instance);  
      }      
      function setTimemodified($timemodified){
        $this->sloodle_ibank_instance->timemodified=$timemodified;  
        $this->timeupdated = time();
        return update_record('sloodle_ibank', $this->sloodle_ibank_instance);  
      } 
      function ibank_makeTransaction($iTransaction){      
        if (insert_record('sloodle_ipoint_trans',$iTransaction)) {
            $this->do_xml_post($this->sloodle_ibank_instance->xmlchannel,"1","UPDATE");
            return true;
        }
        else return false;
      }
    /**
     * @method ibank_getTransactionRecords
     * @author Paul Preibisch
     * 
     ...
     */
      function ibank_getTransactionRecords($userId=null)
      {
          global $CFG,$ibankObj;
         if (!$userId){
            ...
            $sql = 'SELECT * FROM '.$CFG->prefix.'sloodle_ipoint_trans'.
            ' WHERE userid='.$userId.' AND sloodleid='.$ibankObj->sloodleId.
      ...
      function getTotals(){
         global $CFG; 
         $totalCreditsSql='SELECT SUM(amount) as total  FROM '.$CFG->prefix.'sloodle_ipoint_trans'.
                             ' WHERE itype=\'credit\' AND sloodleid='.$this->sloodleId;
         $totalDebitsSql='SELECT SUM(amount) as total FROM '.$CFG->prefix.'sloodle_ipoint_trans'.
                             ' WHERE itype=\'debit\' AND sloodleid='.$this->sloodleId;
         $allTransactionsSql ='SELECT COUNT(id) as total FROM '.$CFG->prefix.'sloodle_ipoint_trans'.
                             ' WHERE sloodleid='.$this->sloodleId;
         ...
         $totals->totalusers= $totalRecs->total;
         return $totals;
      }
     ...
     */
     ...
     function ibank_removeTransaction($userId,$iType){
         return delete_records("sloodle_ipoint_trans",'sloodleid',$this->getSloodleId(),'itype',$iType,'userid',$userId);
     }
      function ibank_updateTransaction($transRec){
        if (!update_record("sloodle_ipoint_trans",$transRec))
     ...
     */
     function getLastTransaction($avuuid,$details)    {
         ...
         $sql='SELECT MAX(id) as id FROM '.$CFG->prefix.'sloodle_ipoint_trans WHERE `idata` LIKE \'%attendance%\' AND `avuuid` =\''.$avuuid.'\''; 
         //get id of user         
         $idRec = get_record_sql($sql);         
         if ($idRec->id!=""){
             $rec = get_record('sloodle_ipoint_trans','id',$idRec->id);            
             return $rec;
         }else {
         ...
         function ibank_getBalanceDetails($userid){
         global $CFG;
         
         $creditsSql = 'SELECT SUM(amount) as total FROM '.$CFG->prefix.'sloodle_ipoint_trans'.
                             ' WHERE itype=\'credit\' AND sloodleid='.$this->sloodleId .
                             ' AND userid='.$userid;
         $debitsSql = 'SELECT SUM(amount) as total FROM '.$CFG->prefix.'sloodle_ipoint_trans'.
                             ' WHERE itype=\'debit\' AND sloodleid='.$this->sloodleId .
                             ' AND userid='.$userid;  
         $balance=0;                           
