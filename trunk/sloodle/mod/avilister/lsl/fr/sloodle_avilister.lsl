// Sloodle AviLister (French version)
...
{   
    ...
    no_sensor()
    {
        llOwnerSay("Pas d'avatars détectés dans les environs.");
        state ready;
    }
    ...
    sensor(integer total_number)
    {
        ...
        llOwnerSay("Demande d'identification de " + (string)total_number + " avatar(s) de: " + sloodleserverroot);
    ...
    http_response(key id, integer status, list meta, string body)
    {
        ...
        if (status != 200) {
            llOwnerSay("ERREUR: échec de la requête HTTP avec le code de statut " + (string)status);
            state ready;
            return;
        }
        ...
        if (statuscode <= 0) {
            // Report the error
            llOwnerSay("Erreur reportée par Moodle (" + (string)statuscode + "): " + dataline);
            state ready;
            return;
        }
        ...
        if (numlines < 2) {
            llOwnerSay("Aucun des avatars détectés n'est reconnu par Moodle.");
            state ready;
            return;
        }
        llOwnerSay("Nombre d'avatars identifiés par Moodle: " + (string)(numlines - 1));
    ...
    timer()
    {
        ...
        llOwnerSay("ERREUR: Temps de recherche d'avatars écoulé.");
