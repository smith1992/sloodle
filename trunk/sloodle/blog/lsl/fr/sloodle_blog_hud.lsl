// Sloodle Blog HUD (French version)
...
show_channel_menu()
{
    ...
    llDialog(llGetOwner(), "utilise actuellement le channel: " + (string)user_chat_channel + ".\nSélectionnez le channel que vous souhaitez utiliser:", ["0", "1", "2", "Annuler"], SLOODLE_CHANNEL_AVATAR_DIALOG);
}
...
{    
    state_entry()
    {
        ...
        if (llGetAttached() < 30)
        {
            llOwnerSay("ERREUR: Veuillez m'atacher en haut de la fenêtre.");
            state error;
        }
    ...
    changed( integer change )
    {
        ...
        {
            //updatepending = TRUE;
            llOwnerSay("Des modifications ont eu lieu... re-initialisation.");
            resetScript();
        }
    }
...
{    
    state_entry()
    {
        ...
        llOwnerSay("Touchez moi pour tenter à nouveau la procedure d'installation.");
        llSetText("Initialisation/identification échouée.\nVeuillez détacher puis ratacher la toolbar , ou remplacez le script de configuration, pour essayer à nouveau.", <1.0,0.0,0.0>, 1.0);
...
{   
    ...
    http_response(key id, integer status, list meta, string body)
    {
        ...
        if (status != 200) {
            llOwnerSay("ERREUR: impossible de vérifier la présence de l'avatar dans la base de données à cause d'un échec de la requète HTTP.");
            state error;
            return;
        }
        ...
        if (statuscode <= 0) {
            // Report the error
            llOwnerSay("Erreur de l'enregistrement de l'utilisateur (" + (string)statuscode + "): " + dataline);
            state error;
            return;
        }
        ...
        if (statuscode == 301) {
            // Nothing to do...
            llOwnerSay("Votre avatar est identifié. Bienvenue.");
        } else {
            // Provide the user a URL with which to authenticate their registration
            llOwnerSay("Votre avatar n'a pas été identifié. Utilisez cette URL pour vous loguer à Moodle.");
    ...
    timer()
    {
        ...
        llOwnerSay("ERREUR: Temps d'attente dépassé pour la réponse HTTP. Déplacement vers l'état error.");
        state error;
    }
...
{   
    state_entry()
    {
        ...
        llOwnerSay("Entrez le sujet de votre message.");
        user_listen_handle = llListen(user_chat_channel, "", llGetOwner(), "");
    ...
    timer()
    {
        ...
        llOwnerSay("Temps d'attente dépassé pour l'entrée du sujet. Annulation du message.");
        state ready;
    }
    ...
    touch_start( integer num )
    {
        ...
        if (name == "send") {
            ...
            llOwnerSay("Impossible d'envoyer votre message maintenant. Entrez dabord le sujet et le message.");
            return;
            
        } else if (name == "cancel") {
            // The "cancel" button was touched
            llOwnerSay("Message annulé.");
            state ready;
    ...
    listen( integer channel, string name, key id, string msg )
    {
        ...
        if (channel == SLOODLE_CHANNEL_AVATAR_DIALOG) {
            ...
            if (msg == "") {
                llOwnerSay("Le message ne peut pas être vide. Entrez le à nouveau.");
                return;
            }
...
{   
    state_entry()
    {
        ...
        // Listen for chat messages from the owner of this object
        llOwnerSay("Entrez le corps de votre message. Vous pouvez utiliser plusieurs chat, pour un total de " + (string)(MAX_BLOG_BODY_LENGTH - blogbodylength - 1) + " caractères.");
        user_listen_handle = llListen(user_chat_channel, "", llGetOwner(), "");
    ...
    {
        // We have timed-out waiting for a response
        llOwnerSay("Temps d'attente dépassé pour l'entrée du message. Annulation du message.");
        state ready;
    }
    ...
    touch_start( integer num )
    {
        ...
        if (name == "send") {
            ...
            if (blogbody == "") {
                llOwnerSay("Tapez un texte avant de sauvegarder votre message.");
                return;
            }
            // Send it
            state send;
    ...
    listen( integer channel, string name, key id, string msg )
    {
        ...
        if (channel == SLOODLE_CHANNEL_AVATAR_DIALOG) {
            ...
            if (blogbodylength >= MAX_BLOG_BODY_LENGTH) {
                llOwnerSay("Désolé. Vous avez atteint votre quota de caractères.");
                return;
            }
            // Make sure the message is not empty
            if (msg == "") return;
            ...
            if ((blogbodylength + msglen) > MAX_BLOG_BODY_LENGTH) {
                llOwnerSay("Désolé -- ce message rendrait votre entrée trop longue. Vous avez seulement " + (string)(MAX_BLOG_BODY_LENGTH - blogbodylength - 1) + " caractères restants.");
                return;
            }
            ...
            if (charsleft > 0) llOwnerSay((string)(charsleft) + " caractères restants.");
            else llOwnerSay("Il ne vous reste plus aucun caractère.");
        }
    }
...
{   
    state_entry()
    {
        ...
        llOwnerSay("Merci! Attendez pendant que le message est envoyé...");
    ...
    timer()
    {
        // We have timed-out waiting for an HTTP response
        llSetTimerEvent(0.0);
        llOwnerSay("ERREUR: Temps ecoulépour la réponse HTTP. Vous pouvez essayer une nouvelle fois, ou annuler.");
        state get_body;
    }
    ...
    http_response(key request_id, integer status, list metadata, string body)
    {
        ...
        // Was the HTTP request successful?
        if (status != 200) {
            llOwnerSay("ERREUR: impossible d'actualiser le blog à cause d'un échec de le requète HTTP. Essayez à nouveau ou annulez.");
            state get_body;
            return;
        }
        ...
        if (statuscode <= -300 && statuscode > -400) {
            ...
            llOwnerSay("Echec de l'actualisation du blog à cause d'une erreur d'identification de l'utilisateur (" + (string)statuscode + "): " + dataline);
            state authenticating;
            return;
            
        } else if (statuscode <= 0) {
            // Don't know what kind of error it was
            llOwnerSay("ERREUR (" + (string)statuscode + "): " + dataline + "\nEssayez à nouveau ou annulez.");
            state get_body;
            return;
        }
        
        // If we get here, then it must have been successful
        llOwnerSay("Actualisation du blog terminée.");
        state ready;
    }
