...
update()
{
    ...
    //this is an experimental version - dont update!
...
submitBlog() {
    ...
    //llOwnerSay("post = " + post);
    
    // llEscapeURL currently returns at most 254 char len string. This meants we need to do a horrible work around
    string msg = "pwd="+PRIM_PASS +"&uuid="+(string)llGetOwner() +"&subject="+llEscapeURL(subject) + "&summary=";
    integer len = llStringLength(post);
    integer i;
    ...
    for (i=0;i<len;i+=STEP)
    {
        ...
        msg = msg + llEscapeURL(llGetSubString(post,i,end));
    }
    //msg = (msg="") + msg +"&summary="+llEscapeURL(post);
        llHTTPRequest(URL_BASE,[HTTP_METHOD,"POST", HTTP_MIMETYPE, "application/x-www-form-urlencoded"],msg);
        //llOwnerSay(msg);
    ...
    dataserver(key query_id, string data) {
        if (query_id == gQueryID) {
            if (data != EOF) {    // not at the end of the notecard
                if (gLine == 2) { // URL_BASE
                URL_BASE = data + "mod/sloodle/blog/sl_blog_linker_post.php";
                //llSay(0, URL_BASE);
                }
            }
        }
